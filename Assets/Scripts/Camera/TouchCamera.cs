...
public class TouchCamera : MonoBehaviour {
	...
	void Awake()
	{
		//GetComponent<Camera> ().transform.rotation = new Quaternion (90, 0, 0,0);
		//GetComponent<Camera> ().transform.localPosition = new Vector3 (12,30,12);
		//GetComponent<Camera> ().orthographicSize = 12;
		
		//yield return new WaitForSeconds (5);
		//GetComponent<Camera> ().transform.rotation = new Quaternion(30, 45, 0,0);
		//GetComponent<Camera> ().transform.localPosition = new Vector3 (-4,3,-4);
		//GetComponent<Camera> ().orthographicSize = 5;
			//-4.3.-4
		//30 45 0
		//GetComponent<Camera> ().GetComponentInParent<AutoCam> ().enabled = true;
	}
	void Update() {
